...
Page({
    ...
    onLoad: function(options) {
        this.getCates()
             ...
             *   1 写代码的方式不一样了
             *      web：localStorage.setItem("key","value") localStorage.setItem("key")
             *      小程序中：wx.getStorageSync("key","value");wx.getStorageSync("key")
             *   2 存的时候 有没有做类型转换
             *      web：不管存入的是 什么类型的数据，最终都会先调用以下 toString(), 把数据变成了字符串 再存入进去
             *      小程序：不存在 类型转换的这个操作 存什么类型的数据进去，获取的时候就是什么类型
             * 1 先判断一下本地存储中有没有旧的数据
             *  {time：Date.now(),data:[...]}
             * 2 没有旧数据 直接发送新的请求
             * 3 有旧的数据 同时 旧的数据也没过期 就使用 本地存储中的旧数据即可
             */
